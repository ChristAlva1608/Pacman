...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return (self.startingPosition,[]) # return a tuple including the starting pos and visited corners list
    ...
    def isGoalState(self, state):
        ...
        st = state[0]
        visitedCorners = state[1] # list of visited corners

        if st in self.corners:
            if not st in visitedCorners:
                visitedCorners.append(st)
    ...
    def getSuccessors(self, state):
        ...
        """
        successors = [] # include available successors (different directions) for current state
        x,y = state[0]
        visitedCorners = state[1] # visited corners list to check if has reached 4 corner or not 
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action) # convert action to vector for adding
            nextx = int(x + dx) # get x coord of next state
            nexty = int(y + dy) # get y coord of next state
            hitsWall = self.walls[nextx][nexty] # check if the next state hits the wall or not
            if not hitsWall:
                # Create a copy of the visited corners list for the successor
                successorVisitedCorners = list(visitedCorners)
                next_node = (nextx, nexty)

                # Check if the next node is a corner and has not already been visited
                if next_node in self.corners and next_node not in successorVisitedCorners:
                    successorVisitedCorners.append(next_node)

                # Define the new state and add it to the successors list
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            ...
            if self.walls[x][y]: # if it hits the wall then it is an illegal move
                return 999999 
